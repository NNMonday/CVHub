...
export default function FavoriteJobs() {
  ...
  const [favoriteJobsList, setfavoriteJobList] = useState([]);
  const fetchJobSeeker = async (req, res) => {
    try {
       const userId = localStorage.getItem("userInfo")
      console.log(userId);
      const response = await axios.get(`http://localhost:9999/api/jobs/favoriteJobs/${userId}`);
      setfavoriteJobList(response.data); // Assuming response.data contains job seeker details
    } catch (error) {
  ...
  return (
    <DashboardLayout>
      <Container>
        <h2>Applied Jobs ({favoriteJobsList.length})</h2>
