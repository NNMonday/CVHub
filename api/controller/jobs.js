...
const insertJob = async (req, res) => {
  const {  tags, jobRole, minSalary, maxSalary, salaryType, education, experience, jobType, vacancies, expirationDate, jobLevel, applyJobOn, description, responsibilities } = req.body;
console.log(req.body);
  try {
    // Ensure all required fields are present
    if ( !minSalary || !maxSalary || !salaryType || !education || !experience || !jobType || !vacancies || !expirationDate || !jobLevel || !applyJobOn || !description || !responsibilities) {
...
const getAppliedJobsForUser = async (req, res) => {
  try {
    const userId = req.params.userId; // Giả sử bạn lấy userId từ params
    const appliedJobs = await jobsRepository.getJobsAppliedByUser(userId);
    res.status(200).json(appliedJobs);
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
};
const getFavoriteJobsByUser = async (req, res) => {
  try {
    const userId = req.params.userId; // Lấy userId từ request parameters
    // Thực hiện query để lấy danh sách công việc yêu thích của người dùng từ cơ sở dữ liệu
    const favoriteJobs = await jobsRepository.getFavoriteJobsByUserId(userId);
